module "batch" {
  ...
  app_settings = {
    APPCONFIG_PATH                                  = format("/storage/appconfig/%s", var.batch_name)
  ...
  app_command_line = format("if ! /home/site/deployments/tools/startup_script.sh; then /storage/tools/%s/startup_script.sh; fi", var.batch_name)
  storage_mounts = [{
    name         = "appconfig"
    type         = "AzureFiles"
...
resource "azurerm_app_service_slot" "batch-release" {
  ...
  site_config {
    app_command_line = format("if ! /home/site/deployments/tools/startup_script.sh; then /storage/tools/%s-release/startup_script.sh; fi", var.batch_name)
    always_on        = "true"
    linux_fx_version = "jbosseap|7-java8"
  }

  app_settings = {
    "APPCONFIG_PATH"                                  = format("/storage/appconfig/%s-release", var.batch_name)
    "TOOLS_PATH"                                      = format("/storage/tools/%s-release", var.batch_name)
    "JAVA_OPTS"                                       = var.java_opts
    "LANG"                                            = var.system_encoding
    "ORACLE_CONNECTION_URL"                           = data.azurerm_key_vault_secret.oracle-connection-url.value
    "ORACLE_SERVER_ADMIN_FULL_NAME"                   = data.azurerm_key_vault_secret.oracle-server-agid-user.value
