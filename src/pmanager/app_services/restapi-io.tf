module "restapi-io" {
  ...
  app_command_line = format("if ! /home/site/deployments/tools/startup_script.sh; then /storage/tools/%s/startup_script.sh; fi", var.restapi_io_name)
...
resource "azurerm_app_service_slot" "restapi-io-release" {
  ...
  site_config {
    app_command_line = format("if ! /home/site/deployments/tools/startup_script.sh; then /storage/tools/%s-release/startup_script.sh; fi", var.restapi_io_name)
    always_on        = "true"
